...
async function callback({ Client, Data, EmailProvider, Database }: CommandExecuteArguments) {
  ...
  try {
    ...
    if (!password) throw new Error("Email not found");

    await EmailProvider.sendEmail({ to: email, subject: "Forget Password", text: "Here is your password please login and reset it! ** " + password + " **" });
    return {
      notification: {
        type: "success",
