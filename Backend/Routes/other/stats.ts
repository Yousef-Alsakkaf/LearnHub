...
async function callback({ Client,Database }: CommandExecuteArguments) {
    ...
    let students = await Database.executeQuery('SELECT COUNT(*) as count FROM users WHERE userType="student"',[]);
    let instructors = await Database.executeQuery('SELECT COUNT(*) as count FROM users WHERE userType="instructor"',[]);
